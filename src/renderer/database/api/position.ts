// API
import { DatabaseApi, MoveApi, BoardApi } from '.';
...
export const countReachablePositionsFromBoard = async (
  ...
  const position = await getOriginalPositionFromBoard(
    board,
    player,
    true,
    dbName
...
export const countReachablePositionsFromPosition = async (
  ...
  while (positionsToCount.length > 0) {
    ...
    positionsToCount = positionsToCount.concat(
      await getPositionsFromCandidateMoves(position, dbName, true, reachedIds)
    );
...
export const getPositionsFromCandidateMoves = (
  position: Position,
  dbName?: string,
  includeCandidateMoves?: boolean,
  filteredPositionIds?: number[]
) => {
  return Promise.all(
    position.candidateMoves
      .map((move) =>
        filteredPositionIds?.includes(move.positionId as number)
          ? undefined
...
export const getOriginalPositionFromBoard = async (
  ...
  const position = await db.transaction(
    ...
    async () => {
      ...
      const returnValue = Object.keys(positionStrings).reduce(
        (acc, current) => {
          return acc
            .or('position')
...
export const getPositionFromBoard = async (
  ...
  const dbPosition = await getOriginalPositionFromBoard(
    board,
    player,
    includeCandidateMoves,
    dbName
  );
...
export const savePosition = async (
  ...
  const dbBoard = await BoardApi.getBoard(board.width, board.height);
  // Create board if it doesn't exist

  const boardId =
    dbBoard?.id ??
    ((await BoardApi.addBoard(board.width, board.height)) as number);
  ...
  return dbPosition?.id ?? addPosition(boardId, originalString, player, ko);
};
